...
def average_duplicates(big_list: list) -> dict:
  barcodes_match = {}
  final = {}
  ee = []
  for i in big_list:
...
def get_reductions(singlecell: pd.DataFrame, axis_id: str, deviations: int) -> pd.DataFrame:
  ...
  singlecell = singlecell.assign(adjust = lambda x: np.ceil(x.passed_filters * (mean / x[axis_id])))
  final = singlecell[['barcode', 'adjust']]
  return final

def get_diag_reductions(singlecell: pd.DataFrame, deviations: int) -> pd.DataFrame:
...
def combine_tables(singlecell: pd.DataFrame, deviations=1):
...
def clean_fragments(fragments_path: str, r_table: dict) -> pd.DataFrame:
  """
  Reduce high tixels by randomly downsampling fragments.tsv according to
  reduction table  
  """
  print("Loading fragments.tsv")
  fragments = pd.read_csv(fragments_path, sep='\t', header=None, skiprows=list(range(51)))
...
reduct_dict = combine_tables(singlecell, deviations=int(args[4]))
